...
def main(page: ft.Page):
    ...
    # ==================== AI 对话流式调用 ====================
    # 在main函数内部顶部增加状态锁（放在current_chart_data等变量附近）
    is_chat_running = False

    # ==================== AI 对话流式调用（增加自动重试） ====================
    async def send_chat_query(e):
        nonlocal chat_history, is_chat_running
        # 防重复点击锁
        if is_chat_running:
            chat_tip.value = "正在处理提问，请稍候！"
            chat_tip.color = ft.Colors.ORANGE_600
            page.update()
            return
        ...
        def get_adaptive_width(page, max_width=400, margin=24, ratio=0.85):
            """获取自适应宽度"""
            if not page.width:
                return 336
            available = page.width - margin
            width = min(available * ratio, max_width)
            return max(width, 200)  # 最小200px
        ...
        for retry_times in range(max_retry):
            ...
            def sync_stream(loop):
                try:
                    ...
                    asyncio.run_coroutine_threadsafe(chunk_queue.put(err_msg), loop)

            # 启动线程执行流式请求
            await asyncio.to_thread(sync_stream, loop)
            ...
            if not stream_error:
                ...
                # 失败，准备重试
                wait_sec = 2 * (retry_times + 1)
                import random
                wait_sec = wait_sec + random.uniform(0,1.2)  # 随机抖动
                retry_label = f"AI服务繁忙，自动等待重试中({retry_times+1}/{max_retry})，等待{wait_sec:.1f}s..."
                chat_tip.value = retry_label
                chat_tip.color = ft.Colors.ORANGE_600
                page.update()
                await asyncio.sleep(wait_sec)
                # 保留已收到文字，不清空气泡，继续下一轮请求
        ...
        try:
            if success:
                ...
                if not full_response.strip():
                    full_response = "AI服务多次繁忙暂时无法响应，请稍后重新发送问题！"
                ai_text_ref.current.value = f"AI占星师：\n{full_response}"
                chat_tip.value = "请求失败，请稍后重试"
                chat_tip.color = ft.Colors.RED
                page.update()
                await scroll_chat_to_bottom()
        finally:
    ...
    def generate_chart(e):
        ...
        if planets:
            ...
            table = ft.DataTable(
                columns=[
                    ft.DataColumn(ft.Text("行星")),
                    ft.DataColumn(ft.Text("星座")),
                    ft.DataColumn(ft.Text("度数")),
                    ft.DataColumn(ft.Text("宫位")),
                    ft.DataColumn(ft.Text("逆行"))
                ],
                rows=rows,
                heading_row_color=ft.Colors.GREY_300
            )
            # 关键：固定宽度容器，强制表格超出屏幕宽度，触发滚动
            fixed_width_container = ft.Container(
                content=table,
                width=720
            )
            # 横向滚动Row
            scroll_wrapper = ft.Row(
            ...
            planet_card = ft.Card(
                content=ft.Container(
                    ft.Column([
                        ft.Text("🪐 行星位置", size=18, weight=ft.FontWeight.BOLD),
                        scroll_wrapper
                    ]),
                    padding=15
                )
        ...
        if aspects:
            ...
            aspect_table = ft.DataTable(
                columns=[
                    ft.DataColumn(ft.Text("星体1")),
                    ft.DataColumn(ft.Text("相位")),
                    ft.DataColumn(ft.Text("星体2")),
                    ft.DataColumn(ft.Text("容许度"))
                ],
                rows=aspect_rows,
                heading_row_color=ft.Colors.GREY_300,
                border_radius=8
            ...
            fixed_width_container = ft.Container(
                content=aspect_table,
                width=560
            )
            scroll_aspect_wrapper = ft.Row(
                [fixed_width_container],
                scroll="horizontal"
            )
            ...
            aspect_card = ft.Card(
                content=ft.Container(
                    ft.Column([
                        ft.Text("🔮 主要相位", size=18, weight=ft.FontWeight.W_700),
                        ft.Divider(height=10),
                        scroll_aspect_wrapper
                    ]),
                    padding=15
                )
            )
